...
proc check_iplevel_settings {mhsinst} {
	  ...
	  # Check maximum supported ID_WIDTH parameter
	  check_ID_WIDTH_settings $mhsinst
...
# Check C_S_AXI_ID_WIDTH parameter settings
#
# Xilinx AXI Interconnect only support up to 32-bit ID_WIDTH

proc check_ID_WIDTH_settings {mhsinst} {

	 set id_width				[xget_hw_parameter_value $mhsinst "C_S_AXI_ID_WIDTH"]
	 
	 if { $id_width > 32 } {
		 error "\n AXI Slave Port ID Width can only be 0 - 32 \n" "" "mdt_error"
	 }
...
proc check_family_parameter_settings {mhsinst} {
     ...
     # qKintex-7
     } elseif { [string compare -nocase "qkintex7" $family] == 0 } {
     	set family "7series"
     	set mhs_family "kintex7"
     ...
     # qVirtex-7
     } elseif { [string compare -nocase "qvirtex7" $family] == 0 } {
     	set family "7series"
     	set mhs_family "virtix7"
     ...
     # qArtix-7
     } elseif { [string compare -nocase "qartix7" $family] == 0 } {
     	set family "7series"
     	set mhs_family "artix7"
     # aArtix-7
     } elseif { [string compare -nocase "aartix7" $family] == 0 } {
     	set family "7series"
     	set mhs_family "artix7"
     ...
     # Zynq
     	...
     	set family "7series"
     # qZynq
     } elseif { [string compare -nocase "qzynq" $family] == 0 } {
     	set family "7series"
     }
  ...
  puts stderr "Warning: \n By default C_PCIE_USE_MODE = \"$gt_use_mode\".\n"        
	}		  
...
proc generate_corelevel_ucf { mhsinst } {
	...
	if { $ref_clk == 0 } {
    		...
    		puts $outputFile "TIMESPEC \"TS_SYSCLK\" = PERIOD \"SYSCLK\" 100.00 MHz HIGH 50 %;"
		if { [xstrncmp $family "7series"] } {
		...
		puts $outputFile "TIMESPEC \"TS_TXOUTCLK\" = PERIOD 100.00 MHz HIGH 50 %;"
    		...
    		puts $outputFile "NET \"$ref_clk_sig\" TNM_NET = \"SYSCLK\";"
    		puts $outputFile "TIMESPEC \"TS_SYSCLK\" = PERIOD \"SYSCLK\" 250.00 MHz HIGH 50 %;"
		if { [xstrncmp $family "7series"] } {
		puts $outputFile "NET \"*/axi_pcie_enhanced_core_top_i/pcie_7x_v1_9_inst/${gt_path}/pipe_wrapper_i/pipe_lane\[0\].gt_wrapper_i/gtx_channel.gtxe2_channel_i/TXOUTCLK\" TNM_NET = \"TXOUTCLK\";"
    ...
    # Virtex-6
        ...
        puts $outputFile "INST \"*${instname}\" AREA_GROUP = \"pblock_PCIe\";"
        if {$fpgatype==240 || $fpgatype==315} then {
            puts $outputFile "AREA_GROUP \"pblock_PCIe\" RANGE=CLOCKREGION_X1Y0:CLOCKREGION_X1Y3;"
        }
        if {$fpgatype==475 || $fpgatype==550} then {
            puts $outputFile "AREA_GROUP \"pblock_PCIe\" RANGE=CLOCKREGION_X1Y2:CLOCKREGION_X1Y5;"
        }
