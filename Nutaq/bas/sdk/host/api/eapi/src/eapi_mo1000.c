 ...
 * @file      eapi_mo1000.c
 *
 * @brief     Function definitions for the eapi MO1000 control
 *
...
Result Mo1000_Presence_send(connection_state * state, unsigned uBoardNum)
{
	...
	cmd_send(state, MO1000_PRESENCE);
...
Result Mo1000_WriteReg_send(connection_state * state, unsigned uBoardNum, Mo1000_eDevices Device, unsigned char ucAddr, unsigned uValue)
{
	...
	cmd_send(state, MO1000_WRITEREG);
	uint_send(state, uBoardNum-1);
	uint_send(state, Device);
	uchar_send(state, ucAddr);
...
Result Mo1000_ReadReg_send(connection_state * state, unsigned uBoardNum, Mo1000_eDevices Device, unsigned char ucAddr, unsigned int *puValue)
{
	...
	cmd_send(state, MO1000_READREG);
	uint_send(state, uBoardNum-1);
	uint_send(state, Device);
	uchar_send(state, ucAddr);
	*puValue = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_GetStatus_send(connection_state * state, unsigned uBoardNum, Mo1000_stBoardStatus *pStatus, Mo1000_stCompareErrors *pCompare)
{
	...
	cmd_send(state, MO1000_GETSTATUS);
	uint_send(state, uBoardNum-1);
    pStatus->m_uRaw = uint_recv(state);
    pCompare->m_uRaw = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_GetTemperature_send(connection_state * state, unsigned uBoardNum, Mo1000_eTempMode Mode, short *psTemp, short *psDac1, short *psDac2)
{
	...
	cmd_send(state, MO1000_GETTEMPERATURE);
	uint_send(state, uBoardNum-1);
	uint_send(state, Mode);
    *psTemp = ushort_recv(state);
    *psDac1 = ushort_recv(state);
    *psDac2 = ushort_recv(state);
	Ret = result_recv(state);
    return Ret;
}
                                  ...
                                  unsigned uFreqMasterClock)
{
	...
	cmd_send(state, MO1000_SETCLOCKCONFIG);
	uint_send(state, uBoardNum-1);
	uint_send(state, SrcClk);
	uint_send(state, uFreqSrcClk);
	uint_send(state, uFreqDacClock);
	uint_send(state, MasterClkMode);
	uint_send(state, uFreqMasterClock);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_WriteClockConfig_send(connection_state * state, unsigned uBoardNum)
{
	...
	cmd_send(state, MO1000_WRITECLOCKCONFIG);
	uint_send(state, uBoardNum-1);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_GetPllConfig_send(connection_state * state, unsigned uBoardNum, Mo1000_stPllParam *pParam)
{
	...
	cmd_send(state, MO1000_GETPLLCONFIG);
	uint_send(state, uBoardNum-1);
    // Deserialize the pll structure
    pParam->uPrimRefFreq = uint_recv(state);
    pParam->uSecRefFreq = uint_recv(state);
    pParam->OutputRef0 = uint_recv(state);
    pParam->OutputRef1= uint_recv(state);
    pParam->OutputRef2 = uint_recv(state);
    pParam->OutputRef3 = uint_recv(state);
    pParam->OutputRef4 = uint_recv(state);
    pParam->uVcoFreq = uint_recv(state);
    pParam->ucPrimaryInputDivider = uchar_recv(state);
    pParam->usReferenceDivider = ushort_recv(state);
    pParam->usInputDivider = ushort_recv(state);
    pParam->usFeedbackDivider = ushort_recv(state);
    pParam->ucBypassDivider = uchar_recv(state);
    pParam->ucPrescaleDivider = uchar_recv(state);
    pParam->aucOutDivider0 = uchar_recv(state);
    pParam->aucOutDivider1 = uchar_recv(state);
    pParam->aucOutDivider2 = uchar_recv(state);
    pParam->aucOutDivider3 = uchar_recv(state);
    pParam->aucOutDivider4 = uchar_recv(state);
    pParam->R2Value = uint_recv(state);
    pParam->R3Value = uint_recv(state);
    pParam->C1Value = uint_recv(state);
    pParam->C2Value = uint_recv(state);
    pParam->C3Value = uint_recv(state);
    pParam->ChargePump = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_SetPllConfig_send(connection_state * state, unsigned uBoardNum, Mo1000_stPllParam *pParamIn)
{
	...
	cmd_send(state, MO1000_SETPLLCONFIG);
	uint_send(state, uBoardNum-1);
    // Serialize the pll structure
    uint_send(state, pParamIn->uPrimRefFreq);
    uint_send(state, pParamIn->uSecRefFreq);
    uint_send(state, pParamIn->OutputRef0);
    uint_send(state, pParamIn->OutputRef1);
    uint_send(state, pParamIn->OutputRef2);
    uint_send(state, pParamIn->OutputRef3);
    uint_send(state, pParamIn->OutputRef4);
    uint_send(state, pParamIn->uVcoFreq);
    uchar_send(state, pParamIn->ucPrimaryInputDivider);
    ushort_send(state, pParamIn->usReferenceDivider);
    ushort_send(state, pParamIn->usInputDivider);
    ushort_send(state, pParamIn->usFeedbackDivider);
    uchar_send(state, pParamIn->ucBypassDivider);
    uchar_send(state, pParamIn->ucPrescaleDivider);
    uchar_send(state, pParamIn->aucOutDivider0);
    uchar_send(state, pParamIn->aucOutDivider1);
    uchar_send(state, pParamIn->aucOutDivider2);
    uchar_send(state, pParamIn->aucOutDivider3);
    uchar_send(state, pParamIn->aucOutDivider4);
    uint_send(state, pParamIn->R2Value);
    uint_send(state, pParamIn->R3Value);
    uint_send(state, pParamIn->C1Value);
    uint_send(state, pParamIn->C2Value);
    uint_send(state, pParamIn->C3Value);
    uint_send(state, pParamIn->ChargePump);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_DoDacCalibration_send(connection_state * state, unsigned uBoardNum)
{
	...
	cmd_send(state, MO1000_DODACCALIBRATION);
	uint_send(state, uBoardNum-1);
	Ret = result_recv(state);
    return Ret;
}
                                    ...
                                    unsigned int *puCalibStatus)
{
	...
	cmd_send(state, MO1000_GETCHANNELCALIBSTATUS);
	uint_send(state, uBoardNum-1);
    *puChannelLaneCalib = uint_recv(state);
    *puChannelFrameCalib = uint_recv(state);
    *puChannelSyncCalib = uint_recv(state);
    *puCalibStatus = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
                                 ...
                                 Mo1000_eOutputState State)
{
	...
	cmd_send(state, MO1000_SETDACOUTCTRL);
	uint_send(state, uBoardNum-1);
	uint_send(state, Channel);
	uint_send(state, State);
	Ret = result_recv(state);
    return Ret;
}
                                     ...
                                     short sOffset)
{
	...
	cmd_send(state, MO1000_SETDACPARDCOFFSET);
	uint_send(state, uBoardNum-1);
	uint_send(state, Channel);
	ushort_send(state, sOffset);
	Ret = result_recv(state);
    return Ret;
}
                                 ...
                                 unsigned char ucGain)
{
	...
	cmd_send(state, MO1000_SETDACPARGAIN);
	uint_send(state, uBoardNum-1);
	uint_send(state, Channel);
	uchar_send(state, ucGain);
	Ret = result_recv(state);
    return Ret;
}
                                          ...
                                          Ad9148_eInterpolationMode Mode)
{
	...
	cmd_send(state, MO1000_SETDACPARINTERPOLATION);
	uint_send(state, uBoardNum-1);
	uint_send(state, Mode);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_DoDacUpdate_send(connection_state * state, unsigned uBoardNum)
{
	...
	cmd_send(state, MO1000_DODACUPDATE);
	uint_send(state, uBoardNum-1);
	Ret = result_recv(state);
    return Ret;
}
                                       ...
                                       Mo1000_eActChannel ActChannel)
{
	...
	cmd_send(state, MO1000_SETDACPARACTCHANNEL);
	uint_send(state, uBoardNum-1);
	uint_send(state, ActChannel);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_GetClkMaster_send(connection_state * state,  unsigned uBoardNum, unsigned int *puMaster)
{
	...
	cmd_send(state, MO1000_GETCLKMASTER);
	uint_send(state, uBoardNum-1);
    *puMaster = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
                             ...
                             short sImgC0, short sImgC1, short sImgC2, short sImgC3, short sImgC4)
{
	...
	cmd_send(state, MO1000_SETDACPARISINC);
	uint_send(state, uBoardNum-1);
	uint_send(state, DacPair);
	ushort_send(state, sRealC0);
	ushort_send(state, sRealC1);
	ushort_send(state, sRealC2);
	ushort_send(state, sRealC3);
	ushort_send(state, sRealC4);
	ushort_send(state, sImgC0);
	ushort_send(state, sImgC1);
	ushort_send(state, sImgC2);
	ushort_send(state, sImgC3);
	ushort_send(state, sImgC4);
                                      ...
                                      Mo1000_eDacGroupId Group, Mo1000_eIsincState State)
{
	...
	cmd_send(state, MO1000_SETDACPARISINCCTRL);
	uint_send(state, uBoardNum-1);
                                    ...
                                    unsigned uFtw, unsigned short usNcoPhaseOffset, Mo1000_eNcoSideband Sideband)
{
	...
	cmd_send(state, MO1000_SETDACPARFINEMOD);
	uint_send(state, uBoardNum-1);
	uint_send(state, Group);
	uint_send(state, uFtw);
	ushort_send(state, usNcoPhaseOffset);
	uint_send(state, Sideband);
                                        ...
                                        Mo1000_eFineModState State)
{
	...
	cmd_send(state, MO1000_SETDACPARFINEMODCTRL);
	uint_send(state, uBoardNum-1);
	uint_send(state, Group);
	uint_send(state, State);
                                       ...
                                       Mo1000_eDataFormat Format)
{
	...
	cmd_send(state, MO1000_SETDACPARDATAFORMAT);
	uint_send(state, uBoardNum-1);
	uint_send(state, Group);
                                 ...
                                 short sRealPhase, short sImaginaryPhase)
{
	...
	cmd_send(state, MO1000_SETDACPARQUADPHASE);
	uint_send(state, uBoardNum-1);
	uint_send(state, DacPair);
	ushort_send(state, sRealPhase);
	ushort_send(state, sImaginaryPhase);
                               ...
                               unsigned int* puCoreVersion, unsigned int *puDriverVersion)
{
	...
	cmd_send(state, MO1000_GETVERSIONS);
	uint_send(state, uBoardNum-1);
    *puCoreVersion = uint_recv(state);
    *puDriverVersion = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_TestLocalOscSw_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTLOCALOSCSW);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
}
...
Result Mo1000_TestBridgeReset_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTBRIDGERESET);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestDacReset_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTDACRESET);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestPllSel_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTPLLSEL);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestPllPowDown_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTPLLPOWDOWN);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestPllSync_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTPLLSYNC);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestDacIrq_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTDACIRQ);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestDacClks_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTDACCLKS);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
...
Result Mo1000_TestDciFrame_send(connection_state * state, unsigned uBoardNum, unsigned int* puResult)
{
	...
	cmd_send(state, MO1000_TESTDCIFRAME);
	uint_send(state, uBoardNum-1);
    *puResult = uint_recv(state);
	Ret = result_recv(state);
    return Ret;
                            ...
                            Mo1000_stI2cAddr *pScanRes, unsigned char *pucDetectOk)
{
	...
	cmd_send(state, MO1000_CHECKI2C);
	uint_send(state, uBoardNum-1);
	uchar_send(state, ucScanMax);
    Ret1 = buf_send(state, (char *)pScanRaw, iScanRawSize);
	uchar_send(state, ucNumDevices);
    Ret2 = buf_recv(state, (char *)pScanRes, sizeof(Mo1000_stI2cAddr));
    *pucDetectOk = uchar_recv(state);
	Ret = result_recv(state);
    if (SUCCESS(Ret))
    {
        if (FAILURE(Ret2))
                                    ...
                                    unsigned short* pusDacRefClock)
{
	...
	cmd_send(state, MO1000_GETCOREFREQUENCY);
	uint_send(state, uBoardNum-1);
    *pusCoreRefClock = ushort_recv(state);
    *pusDacRefClock = ushort_recv(state);
	Ret = result_recv(state);
    return Ret;
}
                               ...
                               unsigned short usEvenPattern, unsigned short usOddPattern)
{
	...
	cmd_send(state, MO1000_SETTESTMODE);
	uint_send(state, uBoardNum-1);
	uint_send(state, Test);
	ushort_send(state, usEvenPattern);
	ushort_send(state, usOddPattern);
	Ret = result_recv(state);
    return Ret;
}
