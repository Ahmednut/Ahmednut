 ...
 ************************************************************************//**  
 * 
 *  @file       : adac250_serialize.c
 *  @brief      : Functions for the serialization of ADAC250 pll parameters
 * 
...
void adac250_pll_param_send(connection_state * state, ADAC250_PLL_PARAM * param)
{
    ...
    uchar_send(state, param->divisorDACInClk);
    uchar_send(state, param->divisorClkOutClk);
    uchar_send(state, param->divisorRsvdClk);
    uchar_send(state, param->divisorDACDataClk);
}
...
void adac250_pll_param_recv(connection_state * state, ADAC250_PLL_PARAM * param)
{
    ...
    param->divisorDACInClk = uchar_recv(state);
    param->divisorClkOutClk = uchar_recv(state);
    param->divisorRsvdClk = uchar_recv(state);
    param->divisorDACDataClk = uchar_recv(state);
}
