...
adp_semaphore_t g_aSem[MAX_RTDEXTS];
...
/************************************************************************************************************************
											   TxTsStreaming 
************************************************************************************************************************/
Result TxTsStreaming(char ** pcIpAddresses, uint32_t NbIpAddresses, handle_streaming handle_txstreaming, handle_rtdexts_channel * handle_txchannel)
{
	adp_result_t res;	
	...
	************************************************************************************************************************/
	...
	for(u32ChNum = 0 ; u32ChNum < u32TotalChNum ; u32ChNum++)
	{
		...
		res = RTDExTs_Open(&g_ahRTDExTs[u32ChNum],                  // Pointer to the RTDExTs handle
                    ...
                    handle_txchannel[u32ChNum]->u8ChannelNumber,    // RTDExTs channel index         
                    RTDExTs_Dir_Downlink,                           // RTDExTs direction
                    uiRTDExBaseAddr,                                // Base address of the RTDEx Ethernet core in the target FPGA.
                    handle_txstreaming->u32SubFrameSize,            // Media frame size in bytes. Limited by the RTDEx media used.
                    handle_txstreaming->u32FrameSize,               // Maximum number of media frames per transfer.
                    handle_txstreaming->u32TsClkFreq,               // Timestamp module clock frequency
                    handle_txstreaming->bTsInit & (u32ChNum == 0),  // Timestamp module initialization flag
                    &handle_txstreaming->u32MaxTransferSize);       // Address of a variable that receives the maximum number of IQ samples
    ...
    for( u32ChNum=0; u32ChNum < u32TotalChNum; u32ChNum++ )
    {
        ...
        res = RTDExTs_SetPropagationDelay( g_ahRTDExTs[u32ChNum], handle_txstreaming->i32PropDelayNs);
        if ( FAILURE(res) )
        {
	...
	************************************************************************************************************************/
	for(u32ChNum = 0 ; u32ChNum < u32TotalChNum ; u32ChNum++)
	{
		...
		res = Buffer_InitQueue( &g_aFreeBuffQ[u32ChNum], handle_txstreaming->u32QueueSize, handle_txstreaming->u32MaxTransferSize * 2*sizeof(int16_t));
        if(FAILURE(res))
		{
			goto done;
		}

		res = Buffer_InitQueue( &g_aFullBuffQ[u32ChNum], 0, handle_txstreaming->u32MaxTransferSize * 2*sizeof(int16_t));
		if(FAILURE(res))
		{
			goto done;
		}
...
static adp_thread_dec_t TxTask( void *param )
{
    ...
    while ( !g_stopTask[u32ChanNum] )
    {
         ...
         * This is done to make sure every thread send the same amount of samples
         * and thread thread does not jam the data pipe while the other thread starve.
         * This must be done since the flow control mechanism is global to all the RTDEx channels.
         */
        WaitSemaphore(&g_aSem[u32ChanNum], 1);

        // Send the data
        res = RTDExTs_Send( hRTDExTs, pBuffDesc->pMem, trSize, NUM_RTDEXTS_EVT, rtdexTsEvtQ, &u32NumRtdexTsEvt );
        if ( u32NumRtdexTsEvt ) DisplayEvent( "TX ", u32ChanNum, u32NumRtdexTsEvt, rtdexTsEvtQ );
        if ( FAILURE(res) ) goto error;
        ...
        // In some cases, we will want to program the stop condition as soon as possible. The sooner we can do it is after the first transfer.
        if ( !chStarted )
        {
            // Pre-program the stop mode if it is OnTrigger or AtXxxTime.
        ...
        // If it is the first transfer, we can optionnally wait for the start condition to happens on the radio before sending more data.
        while ( !chStarted )
        {
            do
            {
                res = RTDExTs_IsStarted( hRTDExTs, NUM_RTDEXTS_EVT, rtdexTsEvtQ, &u32NumRtdexTsEvt, 1000 );
                if ( u32NumRtdexTsEvt ) DisplayEvent( "TX ", u32ChanNum, u32NumRtdexTsEvt, rtdexTsEvtQ );
                if ( FAILURE(res) ) goto error;
            } while( !res );
    ...
    // Optionally, wait for the streams to stop
    if (u32TotalTransfer > 0)
    {
        do
        {
            res = RTDExTs_IsStopped( hRTDExTs, NUM_RTDEXTS_EVT, rtdexTsEvtQ, &u32NumRtdexTsEvt, 5000 );
            if ( u32NumRtdexTsEvt ) DisplayEvent( "TX ", u32ChanNum, u32NumRtdexTsEvt, rtdexTsEvtQ );
            if ( FAILURE(res) ) goto error;
        } while( !res );
    }
...
static adp_thread_dec_t ReadTask( void *param )
{
    ...
    while ( !g_stopTask[u32ChanNum] )
    {
		...
		n = ReadBinFile( fp, pBuffDesc->pMem, hStreaming->u32MaxTransferSize*2*sizeof(int16_t), 1 );
        if ( n < 0 )
        {
            perror( "ReadBinFile:" );
            g_stopTask[u32ChanNum] = 1;
        ...
        // Verify if ReadBinFile returns less bytes than requested
        if ( (uint32_t)n < hStreaming->u32MaxTransferSize*2*sizeof(int16_t))
        {
            //EOF
            break;
        }
