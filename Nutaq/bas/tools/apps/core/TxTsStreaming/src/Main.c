...
static adp_result_t ValidatingInitCall(handle_rtdexts_channel * handle_txchannel);
static adp_result_t FileParser(const char * Filename, handle_streaming * handle_txstreaming, handle_rtdexts_channel * handle_txchannel);
...
int main( int argc, char* argv[] )
{
	...
	************************************************************************************************************************/
	if (argc < 3)
	{
		printf( "Error: Wrong number of parameters\n"
               "Parameters 1 must be the Configuration file name, ex TxTsStreaming.ini,\n"
	...
	************************************************************************************************************************/
	res = TxTsStreaming(&(argv[2]), argc-2, hStreamingMain, hChannel);
	if(FAILURE(res))
	{
		printf("\nError with TxTsStreaming\n");
		Terminate(res);
		return res;
	}
...
static adp_result_t FileParser(const char * Filename, handle_streaming * handle_txstreaming, handle_rtdexts_channel * handle_txchannel)
{
	...
	// Malloc handle
	*handle_txstreaming = (handle_streaming)(malloc(sizeof(sRTDExTsStreamingParams)));
	...
	(*handle_txstreaming)->u32SubFrameSize = u32ValueType;
	...
	(*handle_txstreaming)->u32NumOfSamples = u32ValueType;
	...
	(*handle_txstreaming)->u32TsClkFreq = u32ValueType;
	...
	(*handle_txstreaming)->bTsInit = (uint8_t)u32ValueType;
	...
	(*handle_txstreaming)->i32PropDelayNs = i32ValueType;
	...
	(*handle_txstreaming)->start.mode = (RTDExTs_Mode_t)u32ValueType;
	...
	(*handle_txstreaming)->stop.mode = (RTDExTs_Mode_t)u32ValueType;
    ...
    (*handle_txstreaming)->start.trigSel = (RTDExTs_TrigSel_t)u32ValueType;
	...
	(*handle_txstreaming)->stop.trigSel = (RTDExTs_TrigSel_t)u32ValueType;
	...
	strcpy((*handle_txstreaming)->start.pcTime, pcValueType);
    // Replace '_' by a space character
    for(k=0; k<strlen((*handle_txstreaming)->start.pcTime); k++)
    {
        if(*((*handle_txstreaming)->start.pcTime + k) == '_')
        {
            *((*handle_txstreaming)->start.pcTime + k) = ' ';
        }
    }
    ...
    strcpy((*handle_txstreaming)->stop.pcTime, pcValueType);
    // Replace '_' by a space character
    for(k=0; k<strlen((*handle_txstreaming)->stop.pcTime); k++)
    {
        if(*((*handle_txstreaming)->stop.pcTime + k) == '_')
        {
            *((*handle_txstreaming)->stop.pcTime + k) = ' ';
        }
    }
	...
	for ( i = 0 ; (i < u32NbSections && i < MAX_RTDEXTS) ; i++ )
	{
		...
		// Malloc handle
		handle_txchannel[c] = (handle_rtdexts_channel)(malloc(sizeof(sRTDExTsChannelStruct)));
